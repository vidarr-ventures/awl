...
import type { CrisisContent } from "@/content/crisis";
...
export default function CrisisTemplate({ content }: { content: CrisisContent }) {
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-8 px-6 pb-24 pt-10">
      <FAQJsonLd items={content.faqs} />
      <section className="section-card space-y-4">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink-600">In a Tough Moment</p>
      ...
      {content.sections.map((section) => (
        <section key={section.heading} className="section-card space-y-3">
          <h2 className="text-2xl font-semibold text-ink-900">{section.heading}</h2>
          <div className="prose-awl">
